...
from time import perf_counter  # <-- 추가: 고해상도 타이머
...
# ================== 메인 ==================
def main():
    t_total_start = perf_counter()  # <-- 추가: 전체 실행 시작 시각
    ...
    t_model_start = perf_counter()  # <-- 추가: 모델 호출 시작 시각
    ai_msg = llm.invoke(messages)  # AIMessage
    t_model_end = perf_counter()    # <-- 추가: 모델 호출 종료 시각
    ...
    t_total_end = perf_counter()  # <-- 추가: 전체 실행 종료 시각
    total_runtime = t_total_end - t_total_start

    # ---- 추가 출력: 시간 측정 요약 ----
    print(f"Model latency (invoke→response): {model_latency:.3f} s")
    print(f"Total runtime (start→response saved): {total_runtime:.3f} s")
