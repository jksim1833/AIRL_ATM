...
class BoundingBox(BaseModel):
    x1_percent: float = Field(description="바운딩 박스 왼쪽 끝 위치 (0-100%)")
    y1_percent: float = Field(description="바운딩 박스 위쪽 끝 위치 (0-100%)")
    x2_percent: float = Field(description="바운딩 박스 오른쪽 끝 위치 (0-100%)")
    y2_percent: float = Field(description="바운딩 박스 아래쪽 끝 위치 (0-100%)")

class LuggageAnalysis(BaseModel):
    coordinates: BoundingBox = Field(description="선택된 짐의 바운딩 박스 좌표")
...
3. LOCATE the selected luggage using PERCENTAGE coordinates (0-100%):
   ...
   - CRITICAL: The bounding box must include EVERY SINGLE PART of the luggage
   - Look at the OUTERMOST edges of the luggage in all directions
   - x1_percent: the LEFTMOST edge of ANY part of the luggage (0-100)
   - y1_percent: the TOPMOST edge of ANY part of the luggage (0-100)
   - x2_percent: the RIGHTMOST edge of ANY part of the luggage (0-100)
   - y2_percent: the BOTTOMMOST edge of ANY part of the luggage (0-100)

4. VERIFICATION - Check these requirements:
   - The box extends beyond the luggage edges, not inside them
   - NO part of the luggage (handles, corners, labels, tape) extends outside the box
   - If in doubt, make the box LARGER, not smaller
   - x1_percent should be less than x2_percent
   - y1_percent should be less than y2_percent
   - All percentages should be between 0 and 100

CRITICAL INSTRUCTION: It is better to make the bounding box too large than too small. The box must completely contain the entire luggage with some margin. Think "What are the absolute outer boundaries of this luggage in each direction?"

Example: If luggage spans from 15% to 65% horizontally and 25% to 85% vertically:
{"coordinates": {"x1_percent": 15.0, "y1_percent": 25.0, "x2_percent": 65.0, "y2_percent": 85.0}}

Output format:
{"coordinates": {"x1_percent": number, "y1_percent": number, "x2_percent": number, "y2_percent": number}}"""
    ...
    def draw_bounding_box(self, image_path: Path, coordinates: dict, model_name: str, image_name: str):
        ...
        try:
            ...
            x1 = int(coords['x1_percent'] * width / 100)
            y1 = int(coords['y1_percent'] * height / 100)
            x2 = int(coords['x2_percent'] * width / 100)
            y2 = int(coords['y2_percent'] * height / 100)
            
            print(f"이미지 크기: {width}x{height}")
            print(f"퍼센트 좌표: ({coords['x1_percent']:.1f}%, {coords['y1_percent']:.1f}%, {coords['x2_percent']:.1f}%, {coords['y2_percent']:.1f}%)")
            print(f"픽셀 좌표: ({x1}, {y1}, {x2}, {y2})")
            
            # 빨간색 바운딩 박스 그리기 (두께 3픽셀)
            for i in range(3):
            ...
            # 이미지 저장
            output_file = model_dir / f"{image_name}_bbox.jpg"
            image.save(output_file, 'JPEG', quality=95)
            
            print(f"바운딩 박스 이미지 저장 완료: {output_file}")
...
# Pydantic 모델 정의 (픽셀 기반)
class BoundingBox(BaseModel):
    x1: int = Field(description="바운딩 박스 왼쪽 끝 위치 (픽셀)")
    y1: int = Field(description="바운딩 박스 위쪽 끝 위치 (픽셀)")
    x2: int = Field(description="바운딩 박스 오른쪽 끝 위치 (픽셀)")
    y2: int = Field(description="바운딩 박스 아래쪽 끝 위치 (픽셀)")
...
class LuggageVolumeAnalyzer:
    ...
    def analyze_with_model(self, model_name: str, image_path: Path) -> dict:
        """특정 모델로 이미지 분석"""
        # Pixel-based prompt emphasizing complete inclusion (English)
        prompt = """You are a luggage volume estimation analyst. Analyze the provided image and identify the single piece of luggage that allows for the MOST ACCURATE VOLUME CALCULATION, considering all relevant factors.
...
3. LOCATE the selected luggage using PIXEL coordinates:
   - CRITICAL: The bounding box must include EVERY SINGLE PART of the luggage
   - Look at the OUTERMOST edges of the luggage in all directions
   - x1: the LEFTMOST edge of ANY part of the luggage (in pixels)
   - y1: the TOPMOST edge of ANY part of the luggage (in pixels)
   - x2: the RIGHTMOST edge of ANY part of the luggage (in pixels)
   - y2: the BOTTOMMOST edge of ANY part of the luggage (in pixels)
...
Example: If luggage spans from pixel 150 to 650 horizontally and 200 to 800 vertically:
{"coordinates": {"x1": 150, "y1": 200, "x2": 650, "y2": 800}}
