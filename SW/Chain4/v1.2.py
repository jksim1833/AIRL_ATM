...
class TaskSequenceConverter:
    """
    Task sequence를 16자리 십진수로 변환하는 클래스 v1.2
    - 'unchanged' 함수 지원 추가
    """
    ...
    def __init__(self):
        ...
        self.encoding_rules = {
            ...
            'unfold': '0000',
            'fold': '0001',
            'unchanged': '0000'  # 새로 추가된 unchanged 함수
        }
    ...
    def parse_function_call(self, func_call: str) -> Dict[str, Union[str, int]]:
        ...
        Args:
            func_call: 함수 호출 문자열 (예: "disk_rotate(1, 90)" 또는 "unchanged")
            
        Returns:
            Dict: {'function': 함수명, 'id': id값, 'param': 매개변수}
        """
        # unchanged 함수는 특별 처리 (매개변수 없음)
        if func_call.strip() == 'unchanged':
            return {
                'function': 'unchanged',
                'id': None,
                'param': None
    ...
    def process_cell(self, function_calls: List[str]) -> str:
        ...
        # 각 함수 호출을 인코딩하고 합계 계산, unfold 개수 카운트
...
def main():
    """
    메인 실행 함수 v1.2 - JSON 입력값 처리 (unchanged 함수 지원)
    """
    print("=== Task Sequence to 16-Digit Decimal Converter v1.2 ===")
    print("JSON 형식의 task_sequence 데이터를 입력해주세요.")
    print("지원 함수: disk_rotate, move_on_rail, seat_rotate, unfold, fold, unchanged")
    print("예시 형식:")
    print('{"task_sequence": {"1": ["disk_rotate(1, 90)", "fold(1)", "unchanged"], ...}}')
    print("\n입력 (종료하려면 'quit' 입력):")
                ...
                for cell_id in ['1', '2', '3', '4']:
                    if cell_id in task_sequence:
                        ...
                        for func in functions:
                            ...
                            # unfold 함수 개수 카운트
                            if parsed['function'] == 'unfold':
                                unfold_count += 1
                        
                        print(f"   합계: {' + '.join(encoded_values)} = {total}")
                        print(f"   unfold 함수 등장횟수: {unfold_count}")
                        
                        final = total - unfold_count
                        
                        print(f"   최종 계산: {total} - {unfold_count} = {final}")
