...
def create_scene(disk_type='W'):
    ...
    # 이미지 저장
    filename = f'opencv_{disk_type}_type.png'
    cv2.imwrite(filename, img)
    
    # 이미지 표시
    window_name = f'{disk_type} Type Drawing'
    cv2.imshow(window_name, img)
    
    print(f"{disk_type}타입 구조화된 그림이 생성되었습니다!")
    ...
    print(f"\n{disk_type}타입 구조:")
    if disk_type == 'W':
        print("- disck 함수: 큰 원 + 원 위쪽 작은 원 + 가로선")
    else:
        print("- disck 함수: 큰 원 + 원 우측 작은 원 + 세로선")
