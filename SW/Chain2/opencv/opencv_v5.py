...
def seat(img, rect_center_x, rect_center_y, scale, disk_type='W', position_type='B', direction_type='F'):
    """
    ...
    if direction_type == 'F':  # 북쪽 (위쪽)
        ...
        # 작은 원은 위쪽 꼭짓점에
        small_circle_point = top_point
        
    elif direction_type == 'R':  # 동쪽 (오른쪽)
        # 오른쪽 꼭짓점 (동쪽 방향)
        top_point = (triangle_center_x + triangle_height // 2, triangle_center_y)
        # 왼쪽 위 꼭짓점
        left_point = (triangle_center_x - triangle_height // 2, triangle_center_y - triangle_size // 2)
        # 왼쪽 아래 꼭짓점
        right_point = (triangle_center_x - triangle_height // 2, triangle_center_y + triangle_size // 2)
        # 작은 원은 오른쪽 꼭짓점에
        small_circle_point = top_point
        
    elif direction_type == 'B':  # 남쪽 (아래쪽)
        # 아래쪽 꼭짓점 (남쪽 방향)
        top_point = (triangle_center_x, triangle_center_y + triangle_height // 2)
        # 왼쪽 위 꼭짓점
        left_point = (triangle_center_x - triangle_size // 2, triangle_center_y - triangle_height // 2)
    ...
    small_circle_radius = int(0.3 * scale / 2)
    cv2.circle(img, small_circle_point, small_circle_radius, (255, 0, 255), -1)
...
def create_scene(disk_type='W', position_type='B', direction_type='F'):
    ...
    # 3. seat 함수 호출 (위치와 방향에 따른 삼각형 + 삼각형 위 작은 원)
    seat(img, rect_center_x, rect_center_y, scale, disk_type, position_type, direction_type)
    
    # 이미지 저장
    filename = f'opencv_{disk_type}_{position_type}_{direction_type}_type.png'
    cv2.imwrite(filename, img)
    
    # 이미지 표시
    window_name = f'{disk_type}{position_type}{direction_type} Type Drawing'
    ...
    print(f"{disk_type}{position_type}{direction_type}타입 구조화된 그림이 생성되었습니다!")
    ...
    print(f"스케일: {scale}")
    print(f"\n{disk_type}{position_type}{direction_type}타입 구조:")
    print(f"- disck 함수: 큰 원 + 작은 원 + {disk_names[disk_type]}")
    print(f"- seat 함수: 삼각형({direction_names[direction_type]} 방향) + 작은 원")
