...
class SeatControlChatbot:
    def __init__(self):
        ...
        self.enc = tiktoken.get_encoding("cl100k_base")  # GPT-3.5/4용 인코딩
...
This output seat settings to face the seat forward a new direction that is seat_direction("F")."""
    ...
    def ask(self, prompt):
        ...
        # 입력 토큰 계산
        input_tokens = self.count_tokens(self.chat_history)
    ...
    def count_tokens(self, text):
        """텍스트의 토큰 수 계산"""
        if isinstance(text, list):  # 메시지 리스트인 경우
            total_tokens = 0
            for message in text:
                # 각 메시지는 메타데이터로 인해 추가 토큰이 필요
                total_tokens += 4  # role과 content 등을 위한 토큰
                total_tokens += len(self.enc.encode(message.get("content", "")))
                if message.get("role"):
                    total_tokens += len(self.enc.encode(message["role"]))
            total_tokens += 2  # 전체 메시지 리스트를 위한 토큰
            return total_tokens
        else:
    ...
    def format_metadata(self, input_tokens, output_tokens, response_time):
        ...
        cost_input = input_tokens * 0.0015 / 1000  # GPT-3.5-turbo 입력 토큰당 비용
        cost_output = output_tokens * 0.002 / 1000  # GPT-3.5-turbo 출력 토큰당 비용
        ...
        try:
            ...
            while True:
                ...
                if question in ["!quit", "!exit"]:
                    ...
                    total_cost = (self.total_input_tokens * 0.0015 + self.total_output_tokens * 0.002) / 1000
